"""CSV manager for arithmetic BERT interchange jobs.

This is intentionally smaller than the original interchange_manager.py because
this project only targets arithmetic + BERT and saves raw intervention rows to
CSV.  Every queued job corresponds to one high-level node and one BERT layer.
The worker script expands that into token-location mappings and 100x100
base/source interventions by default.
"""
...
DEFAULT_OPTS = {
    "data_path": "",
    "model_path": "",
    "model_type": "arithmetic_bert",
    "res_save_dir": "",
    "abstraction": "",
    "num_inputs": 100,
    "interchange_batch_size": 64,
    "target_locs": "0 1 2 3 4",
    "device": "cuda",
}


class ArithmeticCSVExperimentManager:
...
def setup(csv_path: str, model_path: str, data_path: str) -> None:
    ...
    ArithmeticCSVExperimentManager(csv_path, opts)
...
def add(
    ...
    manager = ArithmeticCSVExperimentManager(csv_path)
            ...
            row_id = manager.insert(
                {
                    ...
                    "target_locs": " ".join(str(x) for x in target_locs),
                }
            )
...
def run(csv_path: str, script: str, n: Optional[int]) -> None:
    ...
    for row in manager.fetch(n=n, status=STATUS_READY):
        target_locs = row.get("target_locs", "0 1 2 3 4").split()
        cmd = [
            *shlex.split(script),
            "--id", row["id"],
            "--csv_path", csv_path,
...
def main():
    ...
    p.add_argument("--target_locs", type=int, nargs="*", default=[0, 1, 2, 3, 4])
    ...
    if args.cmd == "setup":
        ...
        add(args.csv_path, args.model_path, args.data_path, args.res_dir, args.layers,
            args.high_nodes, args.num_inputs, args.target_locs)
    elif args.cmd == "run":
